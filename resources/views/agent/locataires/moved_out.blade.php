...
<div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="p-8 border-b border-gray-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gray-50/20">
        <div>
            <h2 class="text-2xl font-black text-gray-400 italic">Déménagements Effectués</h2>
            <p class="text-xs text-gray-400 font-bold uppercase tracking-wider mt-1">Historique des locataires ayant quitté leurs logements</p>
        </div>

        <div class="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            <form action="{{ route('agent.locataires.moved_out') }}" method="GET" class="flex items-center gap-2 w-full md:w-auto">
                ...
                @forelse($locataires as $l)
                    <tr class="hover:bg-gray-50/30 transition-colors opacity-75 hover:opacity-100">
                        ...
                        <td class="px-8 py-6 text-center">
                            ...
                            <p class="text-[11px] text-gray-300 font-black uppercase tracking-wider mt-1 italic">{{ $l->profession }}</p>
                        </td>
                        <td class="px-8 py-6 text-center">
                        ...
                        <td class="px-8 py-6 text-center">
                            <div class="flex justify-center">
                                ...
                                <button onclick="showDocuments('{{ $l->name }} {{ $l->prenoms }}', {{ json_encode($availableDocs) }})" 
                                        class="px-4 py-2 bg-gray-50 text-gray-400 border border-gray-100 rounded-xl text-[10px] font-black uppercase tracking-wider hover:bg-gray-200 transition flex items-center gap-2">
                                    <i class="fa-solid fa-box-archive"></i>
                                    Archives ({{ count($availableDocs) }})
                                </button>
                            </div>
                        </td>
                        <td class="px-8 py-6 text-center">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('agent.locataires.reassign', $l->id) }}" 
                    ...
                    <tr>
                        <td colspan="5" class="px-8 py-20 text-center">
                            <div class="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-4">
                                <i class="fa-solid fa-truck-fast text-gray-200 text-2xl"></i>
                            </div>
