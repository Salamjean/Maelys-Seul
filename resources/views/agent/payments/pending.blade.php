...
<div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="p-8 border-b border-gray-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gray-50/20">
        ...
        <div class="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            {{-- Barre de Recherche --}}
            <form action="{{ route('agent.payments.pending') }}" method="GET" class="flex items-center gap-2 w-full md:w-auto">
                <div class="relative w-full md:w-80 group">
                    ...
                    @if(request('search'))
                        <a href="{{ route('agent.payments.pending') }}" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-300 hover:text-red-500 transition-colors">
                ...
                <tr class="hover:bg-gray-50/30 transition-all group">
                    ...
                    <td class="px-8 py-6 text-center">
                        <div class="flex items-center justify-center gap-2">
                            <form action="{{ route('agent.payments.validate', $payment->id) }}" method="POST" id="form-validate-{{ $payment->id }}">
                                @csrf
                            ...
                            <form action="{{ route('agent.payments.cancel', $payment->id) }}" method="POST" id="form-cancel-{{ $payment->id }}">
                                @csrf
