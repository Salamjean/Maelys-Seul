...
<div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="p-8 border-b border-gray-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gray-50/20">
        ...
        <div class="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            {{-- Barre de Recherche --}}
            <form action="{{ route('admin.locataires.index') }}" method="GET" class="flex items-center gap-2 w-full md:w-auto">
                <div class="relative w-full md:w-80 group">
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.locataires.index') }}" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-300 hover:text-red-500 transition-colors">
            ...
            <a href="{{ route('admin.locataires.create') }}" class="px-6 py-3 bg-secondary text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:opacity-90 transition shadow-lg shadow-secondary/10 flex items-center gap-2 whitespace-nowrap">
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse($locataires as $l)
                    <tr class="hover:bg-gray-50/30 transition-colors">
                        ...
                        <td class="px-8 py-6 text-center">
                            @if($l->bien)
                                <a href="{{ route('admin.biens.show', $l->bien->id) }}" class="group">
                                    <p class="text-sm font-black text-primary group-hover:text-secondary transition">{{ $l->bien->reference }}</p>
                        ...
                        <td class="px-8 py-6 text-center">
                            <div class="flex justify-center gap-2">
                                ...
                                <form action="{{ route('admin.locataires.destroy', $l->id) }}" method="POST" onsubmit="return confirm('Supprimer ce locataire ?')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-8 py-20 text-center">
                            ...
                            <a href="{{ route('admin.locataires.create') }}" class="inline-block mt-4 text-xs font-black text-secondary uppercase hover:underline">Ajouter le premier locataire</a>
                        </td>
                    </tr>
                @endforelse
            </tbody>
