...
@section('page-title', 'Édition Agent')
@section('page-subtitle', 'Mise à jour du profil de ' . $agent->name)

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-10">
            <form action="{{ route('admin.agents.update', $agent->id) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" name="residence" value="{{ old('residence', $agent->residence) }}"
                                   class="w-full bg-gray-50 border-2 border-gray-50 focus:border-primary h-14 px-6 rounded-2xl outline-none transition-all font-bold text-sm text-gray-700 shadow-inner"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" name="emergency_contact" value="{{ old('emergency_contact', $agent->emergency_contact) }}"
                                   class="w-full bg-gray-50 border-2 border-gray-50 focus:border-primary h-14 px-6 rounded-2xl outline-none transition-all font-bold text-sm text-gray-700 shadow-inner"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" name="emergency_contact_relation" value="{{ old('emergency_contact_relation', $agent->emergency_contact_relation) }}"
                                   class="w-full bg-gray-50 border-2 border-gray-50 focus:border-primary h-14 px-6 rounded-2xl outline-none transition-all font-bold text-sm text-gray-700 shadow-inner"
                    ...
                    <div class="md:col-span-2 bg-blue-50/50 p-6 rounded-3xl border border-blue-100/50 mb-4">
                        <p class="text-[10px] font-black text-primary uppercase tracking-widest flex items-center gap-2 mb-4">
                            <i class="fa-solid fa-info-circle text-secondary"></i> Modification du mot de passe
                        </p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="text-[10px] font-black text-gray-400 uppercase tracking-[2px] ml-1 block mb-3">Nouveau mot de passe (laisser vide pour ne pas changer)</label>
                                <div class="relative">
                                    <input type="password" name="password"
                                           class="w-full bg-white border-2 border-transparent focus:border-primary h-14 px-6 rounded-2xl outline-none transition-all font-bold text-sm text-gray-700 shadow-sm"
                                           placeholder="••••••••">
                                    <i class="fa-solid fa-lock absolute right-6 top-1/2 -translate-y-1/2 text-gray-300"></i>
                                </div>
                                @error('password') <p class="text-red-500 text-[10px] font-bold mt-2 ml-1 uppercase">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="text-[10px] font-black text-gray-400 uppercase tracking-[2px] ml-1 block mb-3">Confirmer le mot de passe</label>
                                <div class="relative">
                                    <input type="password" name="password_confirmation"
                                           class="w-full bg-white border-2 border-transparent focus:border-primary h-14 px-6 rounded-2xl outline-none transition-all font-bold text-sm text-gray-700 shadow-sm"
                                           placeholder="••••••••">
                                    <i class="fa-solid fa-shield-check absolute right-6 top-1/2 -translate-y-1/2 text-gray-300"></i>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="pt-6 border-t border-gray-50 flex items-center justify-between">
                    ...
                    <button type="submit" class="px-12 h-14 bg-secondary text-white rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-white hover:text-primary transition shadow-xl shadow-secondary/20 flex items-center justify-center gap-3">
                        <i class="fa-solid fa-sync"></i> METTRE À JOUR L'AGENT
                    </button>
                </div>
            </form>
        </div>
