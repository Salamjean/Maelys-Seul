...
class PaymentController extends Controller
{
    ...
    private function calculatePeriod(User $locataire, $monthsToPay)
    {
        ...
        // Formatage textuel (ex: "Janvier 2026" ou "De Janvier à Mars 2026")
    ...
    public function initiateCashPayment(Request $request, User $locataire)
    {
        ...
        // Création de l'enregistrement avec la période
        $payment = Payment::create([
            'user_id' => $locataire->id,
            'months_count' => (int)$request->months,
            'periode_couverte' => $periode, // On enregistre le texte "Mois Année"
            'amount' => $amount,
    ...
    public function confirmByCode(Request $request)
    {
        ...
        if (empty($code)) {
            return response()->json(['error' => 'Code obligatoire.'], 422);
        }
    ...
    public function validateManualPayment(Payment $payment)
    {
        // Génération du QR Code s'il n'existe pas encore
        if (!$payment->qr_code) {
            $qrData = "MAELYS-IMO - RECU DE PAIEMENT\n" .
                      "--------------------------\n" .
                      "Ref: {$payment->reference}\n" .
                      "Locataire: {$payment->user->name} {$payment->user->prenoms}\n" .
                      "Bien: " . ($payment->user->bien->reference ?? 'Logement') . "\n" .
        ...
        $payment->update([
            'status' => 'completed',
            'agent_id' => auth()->guard('admin')->id(), // Enregistre l'admin qui valide
            'paid_at' => now(),
            'verification_code' => null
        ]);

        return back()->with('success', "Le paiement {$payment->reference} a été validé avec succès.");
    }

    public function cancelManualPayment(Payment $payment)
    {
        $payment->update([
            'status' => 'cancelled',
            'agent_id' => auth()->guard('admin')->id() // Enregistre l'admin qui rejette
        ]);

        return back()->with('warning', "Le paiement {$payment->reference} a été annulé.");
    }
    ...
    public function history(Request $request)
    {
        ...
        return view('admin.payments.history', compact('mobilePayments', 'bankPayments', 'cashPayments'));
    }
}
